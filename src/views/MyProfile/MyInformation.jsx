import React, {useEffect, useState} from 'react';
import {StyledMyInformation} from "./MyInformation.styled";
import {getCurrentUser, setCurrentUserAvatar} from "../../utils/auth";
import {EditOutlined, UploadOutlined} from "@ant-design/icons";
import {Button, Input} from "antd";
import {useDispatch} from "react-redux";
import {updateAvatar} from "../../store/user";
import Swal from "sweetalert2";
import axios from 'axios';
// import AWS from 'aws-sdk';

const MyInformation = () => {
    const [selectedFileName, setSelectedFileName] = useState(null);
    const [selectedFile, setSelectedFile] = useState(null);
    const dispatch = useDispatch();
    const [avatarUrl, setAvatarUrl] = useState('');

    useEffect(() => {
        // Lấy giá trị avatar_url từ localStorage và lưu vào state khi component mount
        const storedAvatarUrl = getCurrentUser().avatar_url;
        setAvatarUrl(storedAvatarUrl);
    }, []);

    useEffect(() => {
        setCurrentUserAvatar(avatarUrl)
    }, [avatarUrl]);

    const onChangeAvatar = async (e) => {
        await setSelectedFileName(e.target.files[0].name);
        let reader = new FileReader();

        reader.onload = async function(event) {
            let base64String = event.target.result.split(',')[1];
            setSelectedFile(base64String)
            await handleChangeAvatar(base64String,e.target.files[0].name);
        };

        // Đọc nội dung của file và chuyển đổi thành chuỗi base64
        reader.readAsDataURL(e.target.files[0]);
    }
    const handleChangeAvatar = async (selectedFile, selectedFileName) => {
            const payload = {
                userId: getCurrentUser().id,
                stringAvatar: selectedFile,
                avatarFileName: selectedFileName
            }
            console.log("payload", payload);

            const change_avatar_req = await dispatch(updateAvatar(payload));
            console.log("change avatar request",change_avatar_req);
            if (change_avatar_req.payload.avatarUrl) {
                await Swal.fire(
                    'Thành công!',
                    'Đã cập nhật avatar!',
                    'success'
                )
                setAvatarUrl(change_avatar_req.payload.avatarUrl);
            }
            else {
                alert("Error");
            }
    }
    return (
        <StyledMyInformation>
            <div className="pp">
                {avatarUrl !== null ?
                    <img src={avatarUrl} /> :
                    // <BiUser/>
                    <img
                        src="data:image/png;base64,"
                        style={{borderRadius: '999px'}}
                    />
                }
            </div>
            <div className="tips">
                <UploadOutlined className="upload-outline"/>
                <Input className="input-avatar" type="file" onChange={onChangeAvatar}/>
            </div>
            <div className="information">
                <ul>
                    <li>ID :   <span>{getCurrentUser().id}</span></li>
                    <li>Tên tài khoản :   <span>{getCurrentUser().username}</span></li>
                    <li>Địa chỉ Email :  <span>{getCurrentUser().email}</span></li>
                    <li>Giới tính :  <span>Chưa rõ</span> <EditOutlined style={{fontSize: '20px'}}/></li>
                    <li>Ngày sinh :  <span>Chưa rõ</span> <EditOutlined style={{fontSize: '20px'}}/></li>
                    <li>Điện thoại di động :  <span>Chưa liên kết</span> <a style={{color: 'blue'}}>Đi đến liên kết</a></li>
                </ul>
            </div>
        </StyledMyInformation>
    );
};

export default MyInformation;